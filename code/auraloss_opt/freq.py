...
class SpectralConvergenceLoss(torch.nn.Module):
    ...
    def forward(self, x_mag, y_mag):
        return torch.norm(y_mag - x_mag, p="fro", dim=(-1, -2)) / torch.norm(
            y_mag, p="fro", dim=(-1, -2)
        )
...
class STFTMagnitudeLoss(torch.nn.Module):
    ...
    def __init__(self, log=True, distance="L1", reduction="mean"):
        ...
        self.log = log
        if distance == "L1":
            self.distance = torch.nn.L1Loss(reduction="none")
        elif distance == "L2":
            self.distance = torch.nn.MSELoss(reduction="none")
        else:
    ...
    def forward(self, x_mag, y_mag):
        ...
        loss = self.distance(x_mag, y_mag)
...
class STFTLoss(torch.nn.Module):
    ...
    def forward(self, input: torch.Tensor, target: torch.Tensor):
        ...
        x_and_y = torch.cat(
            [input.view(-1, input.size(-1)), target.view(-1, target.size(-1))]
        )
        if self.perceptual_weighting:
            x_and_y = self.prefilter(x_and_y)
        xy_mag = self.stft(x_and_y)

        if self.scale is not None:
            xy_mag = torch.matmul(self.fb, xy_mag)

        if self.scale_invariance:
            alpha = (x_mag * y_mag).sum([-2, -1]) / ((y_mag**2).sum([-2, -1]))
            y_mag = y_mag * alpha.unsqueeze(-1)

        x_mag, y_mag = xy_mag[: len(xy_mag) // 2], xy_mag[len(xy_mag) // 2 :]
...
class SumAndDifferenceSTFTLoss(torch.nn.Module):
    ...
    def forward(self, input: torch.Tensor, target: torch.Tensor):
        ...
        input_lrms, target_lrms = self.sd(input), self.sd(target)

        # compute error in STFT domain
        loss = self.mrstft(input_lrms, target_lrms)
        total_loss = torch.mean(loss)
        lr_loss = torch.mean(loss[:2])
        ...
        if self.output == "loss":
            ...
            return total_loss, lr_loss, loss[2], loss[3]
